...
export default function Theme2({ItemTypes, aboutItems, skillsItems, setAboutItems, setSkillsItems}){
  ...
  useEffect(()=>{
    ...
    return(
        ...
        <input type="text" placeholder="Enter text" className="heading-input"/>
            ...
            {skillsItems?.map((item, index) => (
              <div key={index} style={{ width: item.type === ItemTypes.PARAGRAPH ? '50%' : item.type === ItemTypes.HEADING ? '100%' : '30%' }}>
                 <div style={{display: 'flex', justifyContent: 'end', alignItems: 'center'}}><img src={images?.crossIcon} className='crossIcon' onClick={()=>{handleRemoveSkillsItem(index)}} /></div>
                     {item.type === 'heading' && <input type="text" placeholder="Enter text" className="heading-input" onChange={(e)=>{handleTextData(e.target.value, 'skill', index)}}/>}
                 {item.type === 'paragraph' && <textarea type="text" placeholder="Enter text" className="paragraph-input-half"  onInput={(e) => e.target.style.height = (e.target.scrollHeight-10) + 'px'} onChange={(e)=>{handleTextData(e.target.value, 'skill', index)}}/>}
          ...
          >
            {aboutItems?.map((item, index) => (
              <div key={index} style={{ width: item.type === ItemTypes.PARAGRAPH ? '50%' : item.type === ItemTypes.HEADING ? '100%' : '30%' }}>
                 ...
                 {item.type === 'heading' && <input type="text" placeholder="Enter text" className="heading-input" onChange={(e)=>{handleTextData(e.target.value, 'about', index)}}/>}
                 {item.type === 'paragraph' && <textarea type="text" placeholder="Enter text" className="paragraph-input"  onInput={(e) => e.target.style.height = (e.target.scrollHeight-10) + 'px'} onChange={(e)=>{handleTextData(e.target.value, 'about', index)}}/>}
